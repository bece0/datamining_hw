...
// https://github.com/rflynn/php-examples/blob/master/stats/agglomerative.php
...
class AGNES{
    ...
    public function __construct(string $dosyaAdi){
        ...
        require_once('DosyaIslemleri.php');

        $this->dosyaIslemleri = new DosyaIslemleri($dosyaAdi);
    }

    public function ParametreleriAyarla(string $xKolon, string $yKolon) : string{
        $hata =  $this->dosyaIslemleri->ParametreleriAyarla($xKolon, $yKolon);
        if($hata != "")
            return $hata;

        $this->dosyaIslemleri->SamplesAyarla();

        return "";
    }

    public function Hesapla(int $number){

        $uzaklikMatrix = $this->dosyaIslemleri->UzaklıkMatrixHesapla();

        $size = count($this->dosyaIslemleri->samples);

        $labels = []; //array_fill(0, $size, 0);
        for ($i=0; $i < $size; $i++) { 
            $labels[$i] = $i;
        }
        ...
        // print("<pre>".print_r($uzaklikMatrix,true)."</pre>");
        // var_dump($uzaklikMatrix); echo "<br>";

        $a = $this->agglomerate($labels, $uzaklikMatrix);

        return $a->splitInto($number);
    }
...
function DosyaOku_UzaklıkMatrisHesapla($dosyaAdi, $xkolon, $ykolon){
    $DosyaData = DosyaOku($dosyaAdi);
